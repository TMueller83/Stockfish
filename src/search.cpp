...
void Search::init() {
  ...
  for (int imp = 0; imp <= 1; ++imp)
      for (int d = 1; d < 128; ++d)
          ...
          {
              double r = 0.00783 * (23.4 + log(Threads.size()) / 2) * d * (1.0 - exp(-8.0 / d)) * log(mc);
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta1 = (prevScore < 0) ? Value(int(20.0 + 0.116 * abs(prevScore))) : Value(26);
              delta2 = (prevScore > 0) ? Value(int(20.0 + 0.116 * abs(prevScore))) : Value(26);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::max(1, int(2.25 * log(depth)) + std::min(int(eval - beta) / 222, 3)); 
